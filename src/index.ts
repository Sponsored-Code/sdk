...
/** Where + how to fund this key's team. `ok:false` when the backend isn't chain-configured. */
export type DepositInfo =
  | { ok: true; distributor: string; usdc: string; chainId: number; network: string; teamRef: string; decimals: number }
  | { ok: false; reason: string };

/** Two EVM calls (approve, then deposit) to sign and broadcast with your own wallet. `value` is always "0x0". */
export type EvmCall = { to: string; data: string; value: "0x0" };
export type DepositPlan = {
  chainId: number;
  usdc: string;
  distributor: string;
  teamRef: string;
  amountMicros: string; // USDC base units (6 decimals), decimal string
  /** approve(distributor, amount) on the USDC token — skip if the standing allowance already covers it. */
  approve: EvmCall;
  /** deposit(teamRef, amount) on the distributor. */
  deposit: EvmCall;
};
...
export class SponsoredCode {
  ...
  /** Bring-your-own-wallet USDC funding: info() says where, depositCalldata() builds the approve + deposit
   *  calls to sign and broadcast with your own signer (ethers / viem / etc.). */
  readonly funding = {
    /** Distributor + USDC addresses, chain, and the team's `teamRef` — or `{ ok:false }` when off-chain. */
    info: (): Promise<DepositInfo> => this.request<DepositInfo>("GET", "/v1/api/deposit-info"),

    /** Build the approve + deposit calls for `amountUsd` USDC. Send `approve` first (skip if the standing
     *  allowance already covers it), then `deposit`. Throws when off-chain. */
    depositCalldata: async (amountUsd: number): Promise<DepositPlan> => {
      const info = await this.funding.info();
      if (!info.ok) throw new SponsoredCodeError("Backend is not chain-configured; no on-chain funding.", 0, info.reason || "chain_not_configured");
      if (!(amountUsd > 0)) throw new SponsoredCodeError("amountUsd must be greater than zero.", 0, "bad_amount");
      const pad32 = (hex: string): string => hex.replace(/^0x/i, "").toLowerCase().padStart(64, "0");
      const micros = BigInt(Math.round(amountUsd * 10 ** info.decimals));
      const amountWord = pad32(micros.toString(16));
      const APPROVE = "0x095ea7b3", DEPOSIT = "0x1de26e16"; // approve(address,uint256) · deposit(bytes32,uint256)
      return {
        chainId: info.chainId, usdc: info.usdc, distributor: info.distributor, teamRef: info.teamRef,
        amountMicros: micros.toString(),
        approve: { to: info.usdc, data: APPROVE + pad32(info.distributor) + amountWord, value: "0x0" },
        deposit: { to: info.distributor, data: DEPOSIT + pad32(info.teamRef) + amountWord, value: "0x0" },
      };
    },
  };
}
